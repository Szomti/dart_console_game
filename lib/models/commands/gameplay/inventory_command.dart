...
class InventoryCommand extends Command {
  ...
  @override
  void execute() {
    final filteredInventory = argsString().isNotEmpty
        ? Player().inventory.where(
              (element) => element.nameId.toLowerCase().contains(argsString()),
            )
        : Player().inventory;
    print('');
    if (filteredInventory.isEmpty) return print('[Empty]');
    final sortedInventory =
        filteredInventory.sorted((a, b) => a.nameId.compareTo(b.nameId));
  ...
  Command fromCommand({required String command}) =>
      InventoryCommand(command: command);
}
